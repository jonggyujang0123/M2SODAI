...
from tools.IPCA_data import HSI_PCA
...
class Resize_JPG_HSI:
    ...
    def __init__(self,
        ...
        if hsi_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.hsi_scale = [hsi_scale]
            assert mmcv.is_list_of(self.hsi_scale, tuple)

        if ratio_range is not None:
            # mode 1: given a scale and a range of image ratio
            assert len(self.img_scale) == 1
            assert len(self.hsi_scale) == 1
        else:
    ...
    @staticmethod
    def random_sample(img_scales): # this function is used
    ...
    def _random_scale(self, results):
        ...
        elif self.multiscale_mode == 'range': # this line is used
            scale, scale_idx = self.random_sample(self.img_scale)
            scale_hsi, scale_idx_hsi = self.random_sample(self.hsi_scale)
        elif self.multiscale_mode == 'value':
            scale, scale_idx = self.random_select(self.img_scale)
            scale_hsi, scale_idx_hsi = self.random_select(self.hsi_scale)
        else:
            raise NotImplementedError

        results['scale'] = scale
        results['scale_hsi'] = self.hsi_scale[0]
        results['scale_idx'] = scale_idx
        results['scale_idx_hsi'] = scale_idx
    ...
    def _resize_hsi(self, results):
        """Resize images with ``results['scale']``."""
        for key in results.get('hsi_fields', ['hsi']):
            if self.keep_ratio:
                img, scale_factor = mmcv.imrescale(
                    results[key],
                    results['scale_hsi'],
            ...
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
            results['hsi_shape'] = img.shape
            # in case that there is no padding
            results['pad_shape_hsi'] = img.shape
            results['scale_factor_hsi'] = scale_factor
            results['keep_ratio_hsi'] = self.keep_ratio
    ...
    def __repr__(self):
        ...
        repr_str += f'(hsi_scale={self.hsi_scale}, '
...
class Pad_JPG_HSI:
    ...
    def __init__(self,
                 ...
                 size_divisor=None,
                 pad_to_square=False,
                 pad_val=dict(img=0, hsi=0,masks=0, seg=255)):
        self.size = size
        self.size_hsi = size_hsi
        ...
        if isinstance(pad_val, float) or isinstance(pad_val, int):
            ...
            pad_val = dict(img=pad_val, hsi=pad_val, masks=pad_val, seg=255)
    ...
    def _pad_hsi(self, results):
        """Pad images according to ``self.size``."""
        pad_val = self.pad_val.get('hsi', 0)
        for key in results.get('hsi_fields', ['hsi']):
            if self.pad_to_square:
                max_size = max(results[key].shape[:2])
                self.size = (max_size, max_size)
            if self.size is not None:
                padded_img = mmcv.impad(
                    results[key], shape=self.size_hsi, pad_val=pad_val)
            elif self.size_divisor is not None:
                padded_img = mmcv.impad_to_multiple(
                    results[key], self.size_divisor, pad_val=pad_val)
            results[key] = padded_img
        results['pad_shape_hsi'] = padded_img.shape
        results['pad_fixed_size_hsi'] = self.size_hsi
        results['pad_size_divisor_hsi'] = self.size_divisor
...
class Normalize_JPG_HSI:
    ...
    def __init__(self, mean, std, to_rgb=True, is_pca = True):
        self.mean = np.array(mean, dtype=np.float32)
        #  self.mean_hsi = np.array(mean_hsi, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        #  self.std_hsi = np.array(std_hsi, dtype=np.float32)
        self.to_rgb = to_rgb
    ...
    def __call__(self, results):
        ...
        #  for key in results.get('hsi_fields', ['hsi']):
            ...
            #  _mean = np.mean(results[key], axis=(0,1), keepdims=True)
            #  results[key] = results[key] - _mean
            #  results[key] = np.abs(results[key])
            #  results[key] = np.tanh(results[key] / 5.0)
        #  results['hsi_norm_cfg'] = dict(
            #  mean=self.mean_hsi, std=self.std_hsi, to_rgb=self.to_rgb)
        return results
...
@PIPELINES.register_module()
class MinIoURandomCrop_JPG_HSI:
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            for i in range(50):
                ...
                patch_hsi = (patch_hsi / hsi.shape[0] * img.shape[0]).astype(int)
                img = img[patch[1]:patch[3], patch[0]:patch[2]]
                results['img'] = img
                results['img_shape'] = img.shape
                hsi = hsi[patch_hsi[1]:patch_hsi[3], patch_hsi[0]:patch_hsi[2]]
                results['hsi'] = hsi
                results['hsi_shape'] = hsi.shape
                # seg fields
                for key in results.get('seg_fields', []):
